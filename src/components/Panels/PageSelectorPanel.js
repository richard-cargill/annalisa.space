...
export default class PageSelectorPanel extends Component {
  ...
  render() {
    ...
    return (<section ref={el => this.elems = el} className="panel pageSelector container">
        <div className="pageSelector__items">
          {pages.map((page, i) => {
            ...
            return (<article className={`pageSelector__item ${isHidden}`} key={name}>
          ...
          {showLoadMoreButton && (
            <div style={{width: '100%'}}>
              <button onClick={this.handleClick} className="button float-right">Load more</button>
            </div>
          )}
        </div>
      </section>
